...
public class JudgeCanPutDown : MonoBehaviour
{
    ...
    public bool canPutDown(GridData[,] grids, int row, int column) {
        //そもそも空きマスでないと置けない
        if (grids[row, column].nowStone != 0)
            return false;
    ...
    public bool canPutDown(GridData[,] grids, int row, int column, int vecRow, int vecColumn) {
        ...
        if (row < 0 || column < 0 || row > GridManager.GRIDSIZE - 1 || column > GridManager.GRIDSIZE - 1)
            return false;
        if (grids[row, column].nowStone == stone)
            return false;
        if (grids[row, column].nowStone == 0)
            return false;
